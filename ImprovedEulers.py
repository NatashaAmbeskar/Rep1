...
Description:Performs Improved Euler's Method given first order differential equation, initial values, increment size, and final x-value.
"""
from sympy import *
...
def ImprovedEulers(diffEQ,x0,y0,xf,step):                              #Improved Euler's method to approximate value of function at xf
    if(step>10**-6 and step<(xf-x0)):                                  #prevents steps that are too small or to big(past the final value)
        ...
        for i in range(iterations):                                    #Loop that performs the Improved Euler's method the number of times necessary to reach xF
            ...
            y0+=(step/2)*(m+diffEQ.subs(x,x0+step).subs(y,y0+m*step))  #recalculates y using average of slope at initial values and values at the end of the step interval(Improved Euler's method)
            x0+=step
        return y0 
    else:
